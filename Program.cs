...
class Program{
    ...
    static readonly string[] parameters = {"Strength", "Stealth", "Magic"}; // Array con i nomi dei parametri associati agli attacchi
    ...
    */
    ...
    static private void CreateNewHero(){  // Funzione per selezionare il nome e la classe di un nuovo personaggio
        ...
        do{
            ...
            Console.Clear();
            Console.WriteLine("Select character's class:");
    ...
    static ExpandoObject NewCharacterSetup(string cClass, [Optional] string name){  // Funzione che assegna i valori ai parametri del personaggio in base alla classe
        ...
        switch(cClass){
            case "Warrior":
                ...
                charObj.parameters[4] = 0;                  // parametro[4] = experience verrà utilizzato con la persistenza dei dati
                break;
            case "Thief":
    ...
    static void Fight(){
        ...
        if(villainObj.parameters[3]<=0){
            Console.Clear();
            Console.WriteLine($"\nYour opposite WIN!!! You are DEAD!!!\n");
            Console.WriteLine("\nPress any key...");
            Console.ReadKey();
        }else{
            Console.Clear();
